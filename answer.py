...
def get_context(inputPrompt, top_k):
    ...
    search_term_vector = get_embedding(
        inputPrompt, engine='text-embedding-ada-002')

    with open("knowledge_base_old.json", encoding='utf-8') as jsonfile:
        data = json.load(jsonfile)
        for item in data:
            item['embeddings'] = np.array(item['embeddings'])

        for item in data:
            item['similarities'] = cosine_similarity(
                item['embeddings'], search_term_vector)

        sorted_data = sorted(
            data, key=lambda x: x['similarities'], reverse=True)
        context = ''
        referencs = []
...
def get_answer(user_input):
    ...
    myMessages = []
    myMessages.append(
        {"role": "system", "content": "You are an expert in your domain. Please provide your expert and user-friendly response based on the context provided. You should aim to provide a clear, concise, and accurate response including contact details."})
